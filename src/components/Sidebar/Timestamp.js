...
class Timestamp extends React.Component {
  ...
  render() {
    ...
    const { vodId, category, topic } = timestamp;

    return(
      <div
        onClick={() => onTimestampClick(vodId, timestamp.timestamp)}
        className={`px-2 py-2 mt-2 mx-2 hover:text-white ${currentTimestamp === timestamp.timestamp && currentVod === vodId? 'bg-grey-darkest' : 'bg-black'} text-grey border border-grey-darkest rounded`}>
        <div className="flex">
          <p
